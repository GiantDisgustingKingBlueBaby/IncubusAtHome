...
if savemanager.GetRunSave() and savemanager.GetRunSave(Familiar).IsMimickedMinisaac == true and not DynamicMinisaacContinued then
return false
...
function DynamicMinisaacLib:InheritTearFlags(entity)
if entity.SpawnerEntity and --DON'T USE PARENT!  I already made the mistake of using Entity.Parent for Lewis's tear scale during K&G's development
entity.SpawnerEntity.Type == EntityType.ENTITY_FAMILIAR
and entity.SpawnerEntity.Variant == FamiliarVariant.MINISAAC then

local minisaac = entity.SpawnerEntity:ToFamiliar()
local player = minisaac.Player

if player and not (player:HasWeaponType(2) or player:HasWeaponType(3) or player:HasWeaponType(4) or player:HasWeaponType(5) or player:HasWeaponType(6) or player:HasWeaponType(9) or player:HasWeaponType(7) or player:HasWeaponType(8)
or player:HasWeaponType(10) or player:HasWeaponType(13) or player:HasWeaponType(14)  ) then
if DynamicMinisaacLib:ShouldNotMimic(entity.SpawnerEntity) == true then return end
--entity:AddTearFlags(player.TearFlags) --Don't do this

if player:HasCollectible(CollectibleType.COLLECTIBLE_CHOCOLATE_MILK) and not (player:HasWeaponType(2) or player:HasWeaponType(3) or player:HasWeaponType(4) or player:HasWeaponType(5) or player:HasWeaponType(6) or  player:HasWeaponType(9) 
or player:HasWeaponType(7) or player:HasWeaponType(8)
or player:HasWeaponType(10) or player:HasWeaponType(13) or player:HasWeaponType(14) ) then

if minisaac.Keys < DynamicMinisaacLib.TearDelayModif.CHOC - 1 then
entity:GetData().RemoveMinisaacTimer = 1;SFXManager():Play(SoundEffect.SOUND_TEARS_FIRE,0)
...
entity.Color = tempEffects.TearColor
entity:GetData().InitComplete = false
entity.TearFlags = tempEffects.TearFlags --Adding TearFlags makes the tears split indefinitely... which is a bug that remains unresolved in kittenchilly's OG ModReference
if entity.TearFlags and TearFlags.TEAR_LUDOVICO ~= 0 then
entity:ClearTearFlags(TearFlags.TEAR_LUDOVICO) --Ludo tear flag makes the tear linger forever
end
if entity.Variant ~= tempEffects.TearVariant then
entity:ChangeVariant(tempEffects.TearVariant)
end
...
function DynamicMinisaacLib:TEARINITLATE(entity) --Code made by peeking TSIL's Post Init Tear Late Callback
if entity:GetData().InitComplete == false then
local minisaac = entity.SpawnerEntity:ToFamiliar()
local player = minisaac.Player
if DynamicMinisaacLib:ShouldNotMimic( entity.SpawnerEntity) == true then return end
local tempEffects = player:GetTearHitParams(WeaponType.WEAPON_TEARS, 1, 1, player)

dmg = ((tempEffects.TearDamage / 3.5) * ((11/30)*(30/(1 + player.MaxFireDelay ))) ) * 1.4

if player:HasCollectible(CollectibleType.COLLECTIBLE_CHOCOLATE_MILK) then
dmg = dmg * 3
entity.Scale = entity.Scale * 2
end

entity.CollisionDamage = dmg

entity:GetData().InitComplete = nil
end
end
...
local baby = player:FireTear(minisaac.Position, entity.Velocity:Rotated((player:GetCollectibleRNG(678):RandomInt(100)%20)-10), false, true, false, minisaac, (entity.CollisionDamage*((player.Damage / 3.5) * ((11/30)*(30/(1 + player.MaxFireDelay ))) ))/(2*player.Damage))
...
brimring.Timeout = 1	
brimring.CollisionDamage = (1.2*entity.CollisionDamage*((player.Damage / 3.5) * ((11/30)*(30/(1 + player.MaxFireDelay ))) ))
...
brimring.Timeout = 1	
brimring.CollisionDamage = (0.4*entity.CollisionDamage*((player.Damage / 3.5) * ((11/30)*(30/(1 + player.MaxFireDelay ))) ))
...
if player and player:HasWeaponType(WeaponType.WEAPON_KNIFE) and minisaac:GetData().IsWieldingBlade and minisaac:GetData().IsWieldingBlade == true then
if DynamicMinisaacLib:ShouldNotMimic( entity.SpawnerEntity) == true then return end
entity:GetData().RemoveMinisaacTimer = 1;SFXManager():Play(SoundEffect.SOUND_TEARS_FIRE,0)
...
if DynamicMinisaacLib:ShouldNotMimic(minisaac) == true then return end
if (player:HasWeaponType(2) or player:HasWeaponType(3) or player:HasWeaponType(4) or player:HasWeaponType(5) or player:HasWeaponType(6)
or player:HasWeaponType(10) or player:HasWeaponType(13) or player:HasWeaponType(14) or player:HasWeaponType(14) ) and minisaac.FireCooldown == 0 then
local sfx = SFXManager()
if sfx:IsPlaying(SoundEffect.SOUND_TEARS_FIRE) then
sfx:Stop(SoundEffect.SOUND_TEARS_FIRE)
end
...
if minisaac then
local player = minisaac.Player
if player and minisaac.Variant == FamiliarVariant.MINISAAC and minisaac.SubType == 99 and player:HasWeaponType(WeaponType.WEAPON_BONE) then
if DynamicMinisaacLib:ShouldNotMimic(minisaac) == true then return end
...
local multbrim = (knife.CollisionDamage*0.25*((player.Damage / 5.25) * ((22/30)*(30/(1 + player.MaxFireDelay ))) ))
...
local brimring3 = Isaac.Spawn(1000, 113, 0, brimring2.Position, brimring2.Velocity, player):ToEffect()
...
brimring.Timeout = 1	
brimring.CollisionDamage = (knife.CollisionDamage*0.25*((player.Damage / 5.25) * ((22/30)*(30/(1 + player.MaxFireDelay ))) ))
...
if not (ItSeemsThatMyTurtleHasDiedNoooo:GetData().ForgorMinisaacBrimBall and ItSeemsThatMyTurtleHasDiedNoooo:GetData().ForgorMinisaacBrimBall == true and ItSeemsThatMyTurtleHasDiedNoooo.Parent) then return end
...
knife.CollisionDamage = (knife.CollisionDamage*((player.Damage / 5.25) * ((22/30)*(30/(1 + player.MaxFireDelay ))) )^0.333333)
...
local multbaby = (entity.CollisionDamage*((player.Damage / 3.5) * ((11/30)*(30/(1 + player.MaxFireDelay ))) ))/(2*player.Damage)
...
local multbrim = (entity.CollisionDamage*((player.Damage / 3.5) * ((11/30)*(30/(1 + player.MaxFireDelay ))) ))/(2*player.Damage)
...
if minisaac.Keys >= delay then
   ...
   local closestEnemy = nil
		...
		local ItSeemsThatMyTurtleHasDiedNoooo1 = Isaac.Spawn(EntityType.ENTITY_EFFECT, 30, 0, minisaac.Position, Vector.Zero, player):ToEffect()
		...
		local ItSeemsThatMyTurtleHasDiedNoooo = Isaac.Spawn(EntityType.ENTITY_EFFECT, 31, 0, minisaac.Position, Vector.Zero, player):ToEffect()
...
if not (ItSeemsThatMyTurtleHasDiedNoooo:GetData().IsMinisaacRocket and ItSeemsThatMyTurtleHasDiedNoooo:GetData().IsMinisaacRocket == true) then return end

if ItSeemsThatMyTurtleHasDiedNoooo.Variant == 30 or ItSeemsThatMyTurtleHasDiedNoooo.Variant == 31 and ItSeemsThatMyTurtleHasDiedNoooo:GetData().IsMinisaacRocket and ItSeemsThatMyTurtleHasDiedNoooo:GetData().IsMinisaacRocket == true then
   ...
   local closestEnemy = nil
		...
		ItSeemsThatMyTurtleHasDiedNoooo.Velocity = (closestEnemy.Position - ItSeemsThatMyTurtleHasDiedNoooo.Position):Normalized() * 30
		else
		ItSeemsThatMyTurtleHasDiedNoooo.Velocity = (closestEnemy.Position - ItSeemsThatMyTurtleHasDiedNoooo.Position)
		end
		end
...
if ItSeemsThatMyTurtleHasDiedNoooo.Variant == 31 and ItSeemsThatMyTurtleHasDiedNoooo:GetData().IsMinisaacRocket and ItSeemsThatMyTurtleHasDiedNoooo:GetData().IsMinisaacRocket == true then
...
if player and not player:HasWeaponType(WeaponType.WEAPON_SPIRIT_SWORD) and not player:HasWeaponType(WeaponType.WEAPON_BONE) then
if minisaac.SubType == 99 then
...
ModReference:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, DynamicMinisaacLib.InheritSwordagain, FamiliarVariant.MINISAAC)
...
if minisaac then
local player = minisaac.Player
if player and minisaac.Variant == FamiliarVariant.MINISAAC and minisaac.SubType == 99 and player:HasWeaponType(WeaponType.WEAPON_SPIRIT_SWORD) and (knife.Variant ~= 10 and knife.Variant ~= 11) then
if DynamicMinisaacLib:ShouldNotMimic( entity.SpawnerEntity) == true then return end
...
local ItSeemsThatMyTurtleHasDiedNoooo = Isaac.Spawn(8, knifevar, 0, knife.Position, Vector.Zero, minisaac):ToKnife()
...
local baby = player:FireTear(minisaac.Position, direction * 10, false, true, false, minisaac, ((11/30)*(30/(1 + player.MaxFireDelay ))))
...
if knife.Parent then
local minisaac = knife.Parent:ToFamiliar()
if minisaac and (knife.Variant == 10 or knife.Variant == 11) and knife:GetData().DisappearMinisaac and knife:GetData().DisappearMinisaac == true then
local player = minisaac.Player
...
if minisaac.Keys >= delay then
	...
	for i = 1, 10 do
local IceOut = Isaac.Spawn(EntityType.ENTITY_TEAR,TearVariant.BLOOD, 0, minisaac.Position, entity.Velocity:Rotated(((player:GetCollectibleRNG(678):RandomInt(40)%40)-20))*(((player:GetCollectibleRNG(678):RandomInt(100)%5)+7)/10), player):ToTear()
...
IceOut:ChangeVariant(tempEffects.TearVariant)
                ...
                IceOut.FallingAcceleration = entity.FallingAcceleration + 0.3*((player:GetCollectibleRNG(678):RandomInt(100)/100))
